...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    fscore = {}
    start_state = problem.getStartState()
    openSet.push((start_state, [], 0), heuristic(start_state, problem))
    gScore[start_state] = 0
    fscore[start_state] = heuristic(start_state, problem)
    while not openSet.isEmpty():
        state, path, cost = openSet.pop()
        if problem.isGoalState(state):
            return path
        for successor, action, stepCost in problem.getSuccessors(state):
            tentative_gScore = gScore[state] + stepCost
            if successor not in gScore or tentative_gScore < gScore[successor]:
                gScore[successor] = tentative_gScore
                fscore[successor] = tentative_gScore + heuristic(successor, problem)
                openSet.push((successor, path + [action], tentative_gScore), fscore[successor])
    util.raiseNotDefined()
